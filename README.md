# Chengyu Wordle | 成语 Wordle

> 🇺🇸 Wordle, for Chinese idioms - 成语 (chéngyǔ). Guess the idiom in 6 tries.
>
> 🇨🇳 Wordle 的中文变种——猜四字成语。玩家有 6 次机会猜出一个成语。

- **🌐 Play**: [wordle-cn.luomor.com](https://wordle-cn.luomor.com/)
- **🇨🇳 中文说明** [（查看）](#中文说明)
...
这是我从头复刻 [Wordle](https://www.nytimes.com/games/wordle/) 的尝试，原版由 [Josh Wardle](https://twitter.com/powerlanguish) 开发。

我偶尔玩 Wordle，觉得挺有意思的。不过个人觉得 Wordle 的*炫耀分享*没啥用，多少污染了社交媒体时间线（Facebook、Twitter），但总比广告强 🔥🤷‍♂️。

后来我看到了一些 Wordle 的"分支"：

- [Primel, by David Lawrence](https://twitter.com/millerdl/status/1484286518005051394) —— 猜 5 位质数
- [Katla, by Fatih Kalifa](https://twitter.com/pveyes/status/1484454960586579968) —— 印尼语版 Wordle
- [Thwordle, by Manassarn Manoonchai](https://twitter.com/narze/status/1484596261277900810) —— 泰语版 Wordle
- [Katapat, by Eugene Low](https://amanz.my/2022322924/) —— 马来语版 Wordle
...
- 26 Feb 2022: **Little Day Out** - [11 Other "Wordle" Challenges to Try](https://www.littledayout.com/11-other-wordle-challenges-to-try/)
- 19 Feb 2022: **Shopee Blog** - [Can't Get Enough Of Wordle? Here's 17 Wordle Variants To Try](https://shopee.sg/blog/best-wordle-games/)
...
- [Node.js](https://nodejs.org/) 14+（支持 ESM）
- [Vite](https://vitejs.dev/)
- [Preact](https://preactjs.com/)
...
- 每个成语都会生成"唯一" ID，但不是 100% 唯一，使用 [shorthash2](https://github.com/jecsham/shorthash2) 生成。我已经测试过，如果用它为所有成语生成 hash，肯定会出现重复。目前只对高频成语（较小的列表）使用。
- 键盘上生成的"键"不保证每个成语持久不变。它们是根据成语列表动态生成的。所以如果成语列表或顺序发生变化，键盘键也会变化。这会影响带有 emoji 的分享结果（🟩🟧⬜）。
- 可以 fork 这个项目并修改所有成语。
...
- 成语列表来自 [pwxcoo/chinese-xinhua](https://github.com/pwxcoo/chinese-xinhua)（经由 [wenhuangzeng/chinese-idiom-chengyu](https://github.com/wenhuangzeng/chinese-idiom-chengyu/blob/master/src/data/idiom.json)）
- 🚨**剧透预警**🚨 [THUOCL（清华开放中文词库）高频成语](http://thuocl.thunlp.org/source/THUOCL_chengyu.txt)——通过 [AllanChain/chinese-wordle](https://github.com/AllanChain/chinese-wordle) 发现。GitHub 仓库：[thunlp/THUOCL](https://github.com/thunlp/THUOCL/blob/master/data/THUOCL_chengyu.txt)

处理后的游戏数据存放在 `game-data/`，运行 `npm run data` 生成：

- `all-idioms.txt` —— 所有成语列表，每行一个
- 🚨**剧透预警**🚨 `game-idioms.csv` —— 游戏中使用的成语列表，带有分配的*"唯一"* `id`，主要来自 THUOCL 高频成语
...
目前中文版本是从英文通过 Google 翻译粗略翻译的，可能不太准确。**欢迎帮忙改进翻译！**

## Other Similar Attempts | 其他类似尝试
...
**A lot more Wordle clones here | 更多 Wordle 克隆**
